...
<h3>Installation</h3>
<ol>
<li>Créez la base de données&nbsp;: ProgAccess.sql contient les requêtes de création des tables de la base de données. Vous pouvez le supprimer après l'avoir importé.</li>
...
<p>L'accès à l'interface d'administration nécessite un compte membre. Le tout premier compte créé a par défaut les droits d'administration. Pour ajouter d'autres membres à l'équipe, vous pouvez passer par la page d'administration "Membres" une fois les comptes créés.</p>
...
<p>Vous avez le droit d'utiliser, d'étudier, de partager et de modifier le code. Mais si vous le publiez, vous devez citer les auteurs (<?php echo $site_name; ?>) et publier le code source sous la même licence ou une autre compatible. Il est également obligatoire de publier le code source modifié si vous l'exécutez publiquement sur un serveur. Évidemment, le simple fait d'ajouter ses identifiants MySQL ou de générer des fichiers de cache via l'interface d'administration ne déclanche pas cette obligation.</p>
<p>Le texte du site (fichiers dans /locales/) est sous licence CC BY-SA. Plus d'informations concernant la licence des traductions, voir le fichier LICENSE.txt dans le dossier locales sur Git.</p>

<h3>Participer</h3>
<a href="<?= GIT_URL ?>">Dépôt GitLab de <?= $site_name ?></a>
...
<p>Pour rapporter un bug ou suggérer une fonctionnalité, vous pouvez <a href="<?= GIT_URL ?>">ouvrir un ticket sur le GitLab</a>, ou utiliser le <a href="/contact_form.php">formulaire de contact</a>.</p>
</main>
