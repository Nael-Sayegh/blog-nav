...
if (isset($_GET['a']) && $_GET['a'] === 's')
{
    ...
    if (empty($log))
    {
        ...
        else
        {
            ...
            {
                $SQL = <<<SQL
                INSERT INTO nav.newsletter_mails (hash, mail, freq_n, freq, notif_site, notif_upd_n, notif_upd, lang, lastmail, lastmail_n) VALUES (:hash, :mail, :frq, :frqn, :notifsite, :notifupd, :notifupdn, :lng, :last, :lastn)
                SQL;
                $req = $bdd->prepare($SQL);
