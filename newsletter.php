...
<html>
	<head>
		<meta charset="utf-8" />
		<title>Confirmation de l\'inscription à la lettre d\'information</title>
	</head>
	<body>
		<div id="header">
		...
		<div id="content">
			<h2>Bonjour</h2>
			<p>Vous avez bien été abonné à la lettre d\'informations de '.$nomdusite.'.</p>
			...
			<p>Cordialement,<br />L\'équipe '.$nomdusite.'</p>
		</div>
	</body>
</html>';

			$msgtext = "Confirmation d'inscription à la lettre d'informations de  $nomdusite (version texte) :
			Bonjour,
Vous avez bien été abonné à la lettre d'informations de $nomdusite.
...
L'équipe $nomdusite";
			...
			$mail->Subject = 'Confirmation de l\'inscription à la lettre d\'informations '.$nomdusite;
			$mail->CharSet = 'UTF-8';
			$mail->IsHTML(TRUE);
			$mail->Body = $message;
			...
			if($mail->send()) {
				...
				$log .= 'Vous êtes bien inscrit à la lettre d\'informations.<br />Veuillez cliquer sur le lien valable 24 heures envoyé à '.$_POST['mail'].' pour confirmer votre inscription.<br />Le mail peut mettre quelques minutes à arriver. Si vous ne le recevez toujours pas, vérifiez dans les indésirables.';
			} else
...
if(isset($_GET['stop']))
	$log .= 'Vous avez bien été désinscrit de la lettre d\'informations. Un mail vous a été envoyé pour confirmer. Vous ne recevrez plus aucun mail de notre part.';
...
<form action="?a=s" method="post">
	...
	<p>Votre adresse e-mail ainsi que toutes vos informations personnelles ne seront pas partagées avec des tiers. Cet abonnement peut être annulé à tout moment. Il sera automatiquement annulé au bout d'un an si vous ne le renouvelez pas (la date d'expiration est affichée en bas de chaque lettre d'information).</p>
