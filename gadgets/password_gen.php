...
<p>Complétez le formulaire ci-dessous pour générer vos mots de passe.</p>
<form method="post">
...
<br>
<label for="f_charpers" id="charpers_lbl" style="display: none">Spécifier les caractères spéciaux (tous collés)&nbsp;:</label>
...
<script>
    document.addEventListener("DOMContentLoaded", () =>
    {
        function showother()
        {
            const sel = document.getElementById("type").value === "5";
            document.getElementById("charpers_lbl").style.display = sel ? "block" : "none";
            document.getElementById("f_charpers").style.display = sel ? "block" : "none";
        }
        showother();
        document.getElementById("type").addEventListener("change", showother);
    });
</script>
</form>
<p id="result">
<?php
if (isset($_POST['nbrPasswd']) && $_POST['nbrPasswd'] <= 10 && isset($_POST['nbrChr']) && $_POST['nbrChr'] <= 250 && isset($_POST['typePasswd']))
{
    $result = '';
    if ($_POST['typePasswd'] === '1')
    {
        $caract = '0123456789';
    }
    elseif ($_POST['typePasswd'] === '2')
    {
        $caract = 'abcdefghijklmnopqrstuvwxyz';
    }
    elseif ($_POST['typePasswd'] === '3')
    {
        $caract = 'abcdefghijklmnopqrstuvwxyz0123456789';
    }
    elseif ($_POST['typePasswd'] === '4')
    {
        $caract = 'abcdefghijklmnopqrstuvwxyz0123456789@!:;,/?*$=+.-_ &)(][{}#"\'';
    }
    elseif ($_POST['typePasswd'] === '5' && isset($_POST['charpers']) && !empty($_POST['charpers']))
    {
        $caract = 'abcdefghijklmnopqrstuvwxyz0123456789'.$_POST['charpers'];
    }
    for ($nbrPasswd = 1; $nbrPasswd <=  $_POST['nbrPasswd']; $nbrPasswd++)
    {
        ...
        {
            if (isset($_POST['maj']) && $_POST['maj'] === 'on' && random_int(0, 2) === 1)
            {
                $password .= strtoupper((string) $caract[mt_rand(0, (strlen((string) $caract) - 1))]);
            }
            else
            {
                $password .= $caract[mt_rand(0, (strlen((string) $caract) - 1))];
            }
        }
        $result .= $password . "\n";
    }
...
<?php
if (isset($_POST['nbrPasswd']) && isset($_POST['nbrChr']) && isset($_POST['typePasswd']))
{ ?>
