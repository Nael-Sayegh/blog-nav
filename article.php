...
while($data = $req->fetch()) {
	...
	echo ($user=getUserById($data['pseudo'])?($user!=false?$user->username.($user->rank=='a'?' ('.tr($tr,'comments_admin').')':''):tr($tr,'empty_nickname')):tr($tr,'empty_nickname'));
