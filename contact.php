...
while($data = $req->fetch()) {
	$teamlist .= '<li style="margin-top:8px;margin-bottom:8px;"><span class="rk rk_a">'.$data['short_name'].'</span> <span style="color:#505050;">(E'.$data['id'].')</span>, '.$data['status'].'<br /><span style="color:#047d04;padding-left:16px;">'.tr($tr, 'teamlist_item', array('age'=>intval((time()-$data['age'])/31557600), 'date'=>date('d/m/Y',$data['date']), 'anniv'=>date('d/m',$data['age']), 'time'=>date('H:i:s',$data['date']))).'</span><p style="margin-top:0;padding-left:16px;color:#202000;">'.$data['bio'].'</p></li>';
