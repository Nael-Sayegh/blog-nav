...
foreach ($langs_prio as &$lang_i)
{
    ...
    $SQL = <<<SQL
        SELECT id,hits FROM softwares ORDER BY hits DESC LIMIT 5
        SQL;
    ...
    foreach ($bdd->query($SQL) as $data)
    {
        fwrite($file, '<p>'.tr($tr, 'last_site_update_text', ['version' => substr((string) $data['name'], 1), 'id' => $data['id'], 'date' => getFormattedDate($data['date'], tr($tr0, 'fndatetime')), 'link1' => '<a href="/u'.$data['id'].'">', 'link2' => '</a>']).'</p>');
    ...
    $SQL = <<<SQL
        SELECT id,author,date FROM softwares ORDER BY date DESC LIMIT 5
        SQL;
    foreach ($bdd->query($SQL) as $data)
    {
        $article_trs = get_article_prefered_tr($data['id'], $lang_i);
        $article_tr = $article_trs['trs'][$article_trs['prefered_tr']];
        fwrite($file, '<li><a href="/a'.$data['id'].'">'.str_replace('{{site}}', $site_name, $article_tr['title']).'</a> ('.tr($tr, 'last_updates_text', ['author' => $data['author'], 'date' => getFormattedDate($data['date'], tr($tr0, 'fndatetime'))]).')</li>');
    }
    fwrite($file, '</ul></div></div>');
    ...
    $SQL = <<<SQL
        SELECT id,downloads FROM softwares ORDER BY downloads DESC LIMIT 5
        SQL;
    foreach ($bdd->query($SQL) as $data)
    {
        $article_trs = get_article_prefered_tr($data['id'], $lang_i);
