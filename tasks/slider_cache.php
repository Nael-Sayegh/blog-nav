...
foreach($langs_prio as &$lang_i) {
	...
	while($data = $req->fetch()) {
		...
		//fwrite($file, '<p>La version '.$data['name'].' ('.$data['id'].') du site est sortie le '.date('d/m/Y',$data['date']).' à '.date('H:i:s',$data['date']).'&nbsp;:<br /><a href="/update.php?id='.$data['id'].'">consultez ses changements</a>.</p>');
	}
	fwrite($file, '</div></div>');
