    ...
    if (isset($_GET['modf2']))
    {
        ...
        if ($data = $req->fetch())
        {
            ...
            if (isset($_POST['social']) && $_POST['social'] === 'on')
            {
                ...
                if ($data = $req->fetch())
                {
                    $somsg = $_POST['title'].' : '.SITE_URL.'/dl/'.(empty($_POST['label']) ?
        ...
        if ($article = $req1->fetch())
        {
            ...
            if ($ok)
            {
                ...
                $SQL = <<<SQL
                        ...
                        SQL;
                $req = $bdd->prepare($SQL);
                $req->execute([':swid' => $_GET['upload'], ':name' => $_POST['name'], ':hash' =>
            }
        }
    }
...
<input type="text" name="label" id="f_modf_label" value="<?= $data['label'] ?>" maxlength="16" readonly>
<?php
        if (!empty($data['label']))
        {
